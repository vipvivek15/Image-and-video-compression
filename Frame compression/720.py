...
#import OpenCV
import cv2
#create video class for analyzing frames
class People:
    #initialize number of rows and columns to be 0
    n_rows=0
    n_cols=0
    #create constructor to initialize path
    def __init__(self,r,c):
        self.path="FourPeople.y4m"
        self.n_rows = r
        self.n_cols = c
    #create Method to obtaain frames from path
    def GetFrame(self):
        ...
        while frameConvert: 
            ...
            if counter<9600: #(600 frames * 16 patches = 9600 patches)
                #read frames
                ...
                for i in range(0,self.n_rows):
                    for j in range(0,self.n_cols):
                        ...
                        cv2.imwrite("Frame(720)\\frame%d.png" % counter, res[i*self.n_cols+j]) 
                        #increment counter until condition fails
                        counter += 1
            else:
                #exits if condition fails
                frameConvert = 0
#create an object of people class        
p = People(4,4)
p.GetFrame()
